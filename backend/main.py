...
class TextValidate():
    ...
    def __init__(self) -> None:
        ...
        self.tokenizer = RobertaTokenizerFast.from_pretrained('roberta-base', longest_first=True)
        self.model = RobertaForSequenceClassification.from_pretrained('roberta-base', return_dict=True, num_labels=2)
        self.model.load_state_dict(load("roberta-base-fever.pth")["model_state_dict"])
        self.model.to(self.device)
        self.nei = RobertaForSequenceClassification.from_pretrained('roberta-base', return_dict=True, num_labels=2)
        self.nei.load_state_dict(load("roberta-NEI-fever.pth")["model_state_dict"])
